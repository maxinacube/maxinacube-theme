...
const ThemeOptions = () => {
	...
	const updateAPI = (path, payload) => {
		...
		apiFetch({
			...
			.then(() => {
				...
				setNoticeMessage(__('Settings saved.', 'maxinacube-theme'));
				window.scrollTo(0, 0);
			})
			.catch((error) => {
	...
	return (
		<div className="maxinacube-theme-options maxinacube-theme-options__main">
			<SocialNetworks updateData={changeData} isOpen />

			<Button className="button button-primary" onClick={submitChanges}>
				{isLoading ? __('Saving...', 'maxinacube-theme') : __('Save', 'maxinacube-theme')}
			</Button>
