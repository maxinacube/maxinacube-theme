...
namespace MaxinacubeTheme\Utility;
...
function get_asset_info( string $slug, string $attribute = null ): array|string|null {
	if ( file_exists( MAXINCUBE_THEME_PATH . 'dist/js/' . $slug . '.asset.php' ) ) {
		$asset = require MAXINCUBE_THEME_PATH . 'dist/js/' . $slug . '.asset.php';
	} elseif ( file_exists( MAXINCUBE_THEME_PATH . 'dist/css/' . $slug . '.asset.php' ) ) {
		$asset = require MAXINCUBE_THEME_PATH . 'dist/css/' . $slug . '.asset.php';
	} else {
		return null;
	}
