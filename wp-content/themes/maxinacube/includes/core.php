...
namespace MaxinacubeTheme\Core;

use MaxinacubeTheme\Utility;
...
function setup(): void {
	...
	add_action( 'init', $n( 'init' ), apply_filters( 'maxinacube_theme_init_priority', 8 ) );
 ...
 */
function init(): void {
	do_action( 'maxinacube_theme_before_init' );

	// If the composer.json isn't found, trigger a warning.
	if ( ! file_exists( MAXINCUBE_THEME_PATH . 'composer.json' ) ) {
		add_action(
			'admin_notices',
			function () {
				$class = 'notice notice-error';
				/* translators: %s: the path to the plugin */
				$message = sprintf( __( 'The composer.json file was not found within %s. No classes will be loaded.', 'maxinacube-theme' ), MAXINCUBE_THEME_PATH );
	...
	do_action( 'maxinacube_theme_init' );
}
 ...
 * If you're building a theme based on "maxinacube-theme", change the
 * filename of '/languages/MaxinacubeTheme.pot' to the name of your project.
 *
 * @return void
 */
function i18n(): void {
	load_theme_textdomain( 'maxinacube-theme', MAXINCUBE_THEME_PATH . '/languages' );
}
...
function theme_setup(): void {
	...
	register_nav_menus(
		[
			'primary' => esc_html__( 'Primary Menu', 'maxinacube-theme' ),
		]
	);
}
...
function scripts(): void {
	...
	wp_enqueue_script(
		'frontend',
		MAXINCUBE_THEME_TEMPLATE_URL . '/dist/js/frontend.js',
	...
	if ( is_page_template( 'templates/page-styleguide.php' ) ) {
		wp_enqueue_script(
			'styleguide',
			MAXINCUBE_THEME_TEMPLATE_URL . '/dist/js/styleguide.js',
	...
	wp_enqueue_script(
		'shared',
		MAXINCUBE_THEME_TEMPLATE_URL . '/dist/js/shared.js',
...
function admin_scripts(): void {
	wp_enqueue_script(
		'admin',
		MAXINCUBE_THEME_TEMPLATE_URL . '/dist/js/admin.js',
...
function core_block_overrides(): void {
	$overrides = MAXINCUBE_THEME_DIST_PATH . 'js/core-block-overrides.asset.php';
	if ( file_exists( $overrides ) ) {
		$dep = require_once $overrides;
		wp_enqueue_script(
			'core-block-overrides',
			MAXINCUBE_THEME_DIST_URL . 'js/core-block-overrides.js',
			$dep['dependencies'],
			$dep['version'],
			true
		);
...
function admin_styles(): void {
	wp_enqueue_style(
		'admin-style',
		MAXINCUBE_THEME_TEMPLATE_URL . '/dist/css/admin.css',
		[],
	 ...
	 * Uncomment this to use the shared.css file.
		wp_enqueue_style(
			'shared-style',
			MAXINCUBE_THEME_TEMPLATE_URL . '/dist/css/shared.css',
			[],
			Utility\get_asset_info( 'shared', 'version' )
		);
	*/
...
function styles(): void {
	wp_enqueue_style(
		'styles',
		MAXINCUBE_THEME_TEMPLATE_URL . '/dist/css/frontend.css',
		[],
		Utility\get_asset_info( 'frontend', 'version' )
	);
	...
	if ( is_page_template( 'templates/page-styleguide.php' ) ) {
		wp_enqueue_style(
			'styleguide',
			MAXINCUBE_THEME_TEMPLATE_URL . '/dist/css/styleguide.css',
			[],
...
/**
 * Remove assets from pages where they are not in use.
 *
 * @return void
 */
function assets_cleanup(): void {
	if ( ! is_admin() && ! has_block( 'gravityforms/form' ) ) {
		wp_dequeue_script( 'jquery' );
		wp_deregister_script( 'jquery' );
		wp_dequeue_style( 'custom-gravity-style' );
	...
	if ( file_exists( MAXINCUBE_THEME_BLOCK_DIST_DIR ) ) {
		$block_json_files = glob( MAXINCUBE_THEME_BLOCK_DIST_DIR . '*/block.json' );

		foreach ( $block_json_files as $filename ) {
			$block_json_content = file_get_contents( $filename ); // phpcs:ignore
			$block_data         = json_decode( $block_json_content, true );

			if ( isset( $block_data['name'] ) ) {
				$block_name = $block_data['name'];
				if ( ! has_block( $block_name ) ) {
					$script_handle = str_replace( '/', '-', $block_name ) . '-script';
